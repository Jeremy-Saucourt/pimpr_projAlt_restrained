function [ transmittance ] = objDiffracAnneaux( objDiffrac, CP, grid )
...
%       * transmittance : transmittance complexe de l'objet diffractant
    ...
    for it=1:CP.NB
%         ind1 = find( (grid.x-CP.posFaisc.x(it)).^2+(grid.y-CP.posFaisc.y(it)).^2 >= (objDiffrac.rayAnneau - objDiffrac.deltaRayAnneau/2).^2 ) ;
%         ind2 = find( (grid.x-CP.posFaisc.x(it)).^2+(grid.y-CP.posFaisc.y(it)).^2 <= (objDiffrac.rayAnneau + objDiffrac.deltaRayAnneau/2).^2 ) ;
        ind1 = find( (grid.x-CP.posFaisc.x(it)).^2+(grid.y-CP.posFaisc.y(it)).^2 >= (objDiffrac.rayAnneau).^2 ) ;
        ind2 = find( (grid.x-CP.posFaisc.x(it)).^2+(grid.y-CP.posFaisc.y(it)).^2 <= (objDiffrac.rayAnneau + objDiffrac.deltaRayAnneau).^2 ) ;
...
%                                         ) ;
            ...
            courbPhase(ind{it}) = ( 2*pi/CP.lambda*...
                                        ( ...
                                            (grid.x(ind{it})-CP.posFaisc.x(it)).^2 ...
                                            + (grid.y(ind{it})-CP.posFaisc.y(it)).^2 ...
                                        ) ...
